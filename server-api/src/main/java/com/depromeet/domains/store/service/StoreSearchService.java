...
public class StoreSearchService {
    ...
    public StoreSearchResponse searchStoreList(User user, String query, String x, String y, Optional<Integer> storePage, Optional<Integer> cafePage) {
        ...
        if (storePage.isPresent()) {
            // 음식점 검색 API URI 생성
            UriComponentsBuilder builderStore = UriComponentsBuilder.fromHttpUrl("https://dapi.kakao.com/v2/local/search/keyword.json")
        ...
        if(cafePage.isPresent()) {
            // 카페 검색 API URI 생성
            UriComponentsBuilder builderCafe = UriComponentsBuilder.fromHttpUrl("https://dapi.kakao.com/v2/local/search/keyword.json")
